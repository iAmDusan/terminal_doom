...
# More detailed and interesting map
...
def main(stdscr):
    ...
    if curses.has_colors():
        curses.start_color()
        curses.init_pair(1, curses.COLOR_WHITE, curses.COLOR_BLACK)     # Normal walls
        ...
        curses.init_pair(10, curses.COLOR_BLACK, curses.COLOR_YELLOW)   # Decorative elements
    ...
    while running:
        ...
        if key != -1:
            if key == ord('x'):
                ...
                if 0 <= int(new_y) < len(MAP) and 0 <= int(new_x) < len(MAP[0]) and MAP[int(new_y)][int(new_x)] != '#':
                        ...
                        if enemy_hit:
                            ...
                            if y >= enemy_ceiling and y <= enemy_floor:
                                ...
                                if hit_wall:
                                    if boundary:
                                        # Wall edge
                                        stdscr.addch(y + 1, x, '|', curses.color_pair(1))
                                    else:
                                        ...
                                        if corrected_distance < DEPTH / 4:
                                            ...
                                            char = '▓'
                                        elif corrected_distance < DEPTH * 3 / 4:
                                            char = '▒'
                                        else:
                                            char = '░'

                                        # Apply appropriate color based on wall type
                                        if hit_lower_wall:
                                            color = curses.color_pair(9)  # Lower wall
                                        else:
                                            # Different shading for NS vs EW walls
                                            if wall_orientation == 1:  # Horizontal wall (darker)
                                                color = curses.color_pair(1)
                                            else:  # Vertical wall (lighter)
                            ...
                            # Add edge shading
                            if boundary:
                                color = curses.color_pair(2)  # Darker for edges
